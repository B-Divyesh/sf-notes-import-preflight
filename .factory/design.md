...
- **Type:** headings use the self-hosted geometric display face Space Grotesk; reports and controls use the self-hosted practical sans face Atkinson Hyperlegible. Latin WOFF2 subsets are sourced from Fontsource under the SIL Open Font License and total about 57 KB. Monospace data uses the local UI-monospace stack, with tabular figures. The pairing feels like hand-lettered signage above an exacting manifest.
...
## Repair 2 application

- Public copy now names notes, exports, folders, reports, and comparisons directly. The visual reference remains in shape, color, and texture rather than metaphorical headings.
- The demo uses a paper report surface inside the existing dark inspection layout. A red edge and explicit “Loss found” wording distinguish the sample state without relying on color.
- The 404 page uses the same oversized type, ink background, mint return action, and small red status code.
- `assets/terminal-demo.svg` is an original project-owned vector recording composed from the actual `notes-preflight demo` output on 6 September 2026. It uses no third-party inputs.
- `og-image.jpg` is a 1200×630 center crop of the original generated hero. `apple-touch-icon.png` is a 180×180 crop from the same project-owned asset. Both were made locally with ImageMagick on 6 September 2026.
